function findByName (nodes, name) { // eslint-disable-line
...
function addKillBtn (node, data) { // eslint-disable-line
...
function uptime (data) { // eslint-disable-line
  ...
  var h = '0' + Math.floor((uptime - d * cd) / ch)

  var m = '0' + Math.round((uptime - d * cd - h * ch) / 60000)
  if (d > 0) {
    status = d + 'd ' + h.substr(-2) + 'h' // eslint-disable-line
  } else {
    status = h.substr(-2) + 'h ' + m.substr(-2) + 'm' // eslint-disable-line
  }
  return status
}
...
JMX.TimeSeries = (function () {
  ...
  Constr = function (container, labels, properties, propertyMaxY, unitY) { // eslint-disable-line
  ...
  Constr.prototype.update = function (data) {
    ...
    for (var c = 0; c < this.properties.length; c++) {
      var val = getProperty(data, this.properties[c])
      if (this.unitY === 'mb') {
